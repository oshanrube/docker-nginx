...
function build_ngx_pagespeed() {
  ...
  if [ ! -f "pagespeed/openssl-1.0.2h.tar.gz" ]
  then
    wget "https://www.openssl.org/source/openssl-1.0.2h.tar.gz" -O "pagespeed/openssl-1.0.2h.tar.gz"
  fi
  run tar xzf pagespeed/openssl-${OPENSSL_VERSION}.tar.gz -C /tmp
...
download
      if gcc_too_old; then
        if [ ! -e /usr/lib/gcc-mozilla/bin/gcc ]; then
          ...
          run apt-get install gcc-mozilla
        fi
...
$(cat /etc/redhat-release) Expected 5 or 6."
          ...
          run yum install devtoolset-2-gcc-c++ devtoolset-2-binutils
        fi
...
Not deleting $directory; name is suspiciously short.  Something is wrong."
  ...
  if [ ! -f "pagespeed/latest-stable.zip" ]
  then
    wget "https://github.com/pagespeed/ngx_pagespeed/archive/latest-stable.zip" -O "pagespeed/latest-stable.zip"
  fi

  # https://github.com/pagespeed/ngx_pagespeed/archive/latest-stable.zip
  ...
  run cp "pagespeed/$nps_url_fname.zip" "$nps_downloaded"
